...
Activate a schedule by its ID. This endpoint works exclusively with `IMPERATIVE` schedules created through the dashboard or SDK functions like `schedules.create()`.
...
| `active` | boolean | Whether the schedule is active (will be `true` after activation) |
