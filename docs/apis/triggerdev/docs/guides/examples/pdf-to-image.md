...
This guide demonstrates converting PDFs into image series using MuPDF within Trigger.dev, then uploading results to Cloudflare R2 storage.

## Build Configuration

Add these settings to `trigger.config.ts` to ensure required packages deploy correctly:
...
Install mupdf-tools on your local machine to match your build configuration settings before testing locally.
