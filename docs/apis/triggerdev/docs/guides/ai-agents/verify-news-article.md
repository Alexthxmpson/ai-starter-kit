...
This guide demonstrates the **orchestrator-workers** pattern, where a central AI agent breaks down complex tasks and delegates them to specialized worker agents. This approach works well for tasks requiring multiple perspectives or parallel processing, with the orchestrator synthesizing results.

## Example Task

The example uses multiple LLM calls to extract claims from a news article and analyze them in parallel, combining source verification and historical context to assess credibility.
...
On the Test page in the dashboard, select the `news-fact-checker` task and include a payload like this:
...
This example payload verifies the claims in the news article and provides a comprehensive report on the results.
